...
for trial in tqdm(range(trials)):
    ...
    if turn==1: # player 1 won
        ...
        for state, action, new_state in moves_minus_1:
            q_table_full[state, action] = q_table_full[state, action] + alpha*(-1*post_reward + gamma*np.max(q_table_full[new_state]) - q_table_full[state, action])
    if turn==-1: # player -1 won
        for state, action, new_state in moves_1:
            q_table_full[state, action] = q_table_full[state, action] + alpha*(-1*post_reward + gamma*np.max(q_table_full[new_state]) - q_table_full[state, action])
        for state, action, new_state in moves_minus_1:
...
for trial in tqdm(range(trials)):
    ...
    if turn==1: # player 1 won
        for state, action, new_state in moves_1:
            q_table_sparse[state, action] = q_table_sparse[state, action] + alpha*(post_reward + gamma*np.max(q_table_sparse[new_state]) - q_table_sparse[state, action])
        for state, action, new_state in moves_minus_1:
            q_table_sparse[state, action] = q_table_sparse[state, action] + alpha*(-1*post_reward + gamma*np.max(q_table_sparse[new_state]) - q_table_sparse[state, action])
    if turn==-1: # player -1 won
        for state, action, new_state in moves_1:
            q_table_sparse[state, action] = q_table_sparse[state, action] + alpha*(-1*post_reward + gamma*np.max(q_table_sparse[new_state]) - q_table_sparse[state, action])
        for state, action, new_state in moves_minus_1:
...
if optimal_table_full:
    plt.axvline(x=50, color='r', linestyle='--', linewidth = 3,label=f"optimal policy learned at iteration {50}")
if optimal_table_sparse:
    plt.axvline(x=102, color='blue', linestyle='--', linewidth = 3,label=f"optimal policy learned at iteration {102}")
