...
trials = 1000
batch_size = 10  # n episodes per batch
scores = [[] for i in range(trials // batch_size)]
for trial in range(trials):
    state, player_flag = env.reset()
    reward = 0
    done = False
    game_memory = []
    while not done:
    ...
    model = replay_game(replay_memory)
...
plt.plot(x, y_total, label=f"Average score over batches of {batch_size} episodes")
plt.xlabel("Trials")
